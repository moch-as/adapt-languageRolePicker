...
export default class LanguagePickerView extends Backbone.View {
  ...
  clearUnselectedItems(languageDropDown, rolesDropDown) {
    ...
    if (!languageIDs?.some(language => language._isSelected) && !languagePlaceholder?.length && languageDropDown?.selectedIndex) languageDropDown.selectedIndex = -1;
    const roles = this.model.get('_roles');
    const rolePlaceholder = this.model.get('_roleOptionsPlaceholder');
    if (!roles?.some(role => role._isSelected) && !rolePlaceholder?.length && rolesDropDown?.selectedIndex) rolesDropDown.selectedIndex = -1;
